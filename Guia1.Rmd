...
salidaPerceptron <- entrenarPerceptron(OR_trn, maxEpocas = 10, critFinalizacion = 0.9)
...
```{r}
graficarRectaSeparacion(salidaPerceptron$W, OR_trn)
```
...
```{r}
test <- aplicarPerceptron(salidaPerceptron$W, OR_tst)
test$tasa
```
...
salidaPerceptron <- entrenarPerceptron(XOR_trn, maxEpocas = 10, critFinalizacion = 0.8)
salidaPerceptron
...
```{r}
graficarRectaSeparacion(salidaPerceptron$W, XOR_trn)
```
...
```{r}
test <- aplicarPerceptron(salidaPerceptron$W, XOR_tst)
test$tasa
```
...
for (n in seq(1,5)) {
  ...
  #Entrenamiento
  salidaPerceptron <- entrenarPerceptron(spheres1d10[particion_spheres1d10$trn[[n]],], 
...
resultado <- entrenarPerceptronM(concentlite, maxEpocas = 500, critFinalizacion = 0.9, nu=0.1,
                                 arquitectura = c(3,1), imprimir = FALSE)
...
resultado2 <- entrenarPerceptronM(concentlite[,1:3], maxEpocas = 500, critFinalizacion = 0.9, nu=0.1,
                                 arquitectura = c(3,1), alfa = 0.5, imprimir = FALSE)
...
irisbin$clase <- 0
irisbin[which(irisbin$X7==1),]$clase <- 1
irisbin[which(irisbin$X6==1),]$clase <- 2
irisbin[which(irisbin$X5==1),]$clase <- 3

irisbin2 <- irisbin[,-c(5,6,7)]
...
```{r}
resultadoIris <- entrenarPerceptronM(irisbin2, maxEpocas = 20000, critFinalizacion = 0.9, nu=3,
                                 arquitectura = c(4,3), alfa = 1)
# hay que revisar, no anda
```
